...
def IQAvectors(file_name, n_max=None, descriptor='all', verbose=False, **kwargs):
	...
	### atomic electronic kinetic energy descriptor from .sum file.
...
def _atomic_energy_sum_descriptor_from_dict(ke_atoms, atom_type_list):
...
# 	n_atoms = numline2 - numline1 - 5
	...
	try:
		for i in range(n_atoms):
			t = sum_file.readline(); lineDecomp = t.split()
# 			if 'D' in lineDecomp[3]: # @todo: to change back
# 				print(lineDecomp[3])
...
# 		print('The "Some Atomic Properties" part is not shown in the .sum file: ' + repr(e))
	...
	try:
		...
		print('Eint_matrix not corrected extracted: ' + repr(e))
		raise e
	except ValueError as e:
		raise ValueError('Eint_matrix not corrected extracted: ' + repr(e))
